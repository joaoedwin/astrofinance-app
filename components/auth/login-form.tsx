...
export function LoginForm() {
  ...
  return (
    <div className="flex min-h-[80vh] w-full max-w-5xl rounded-xl overflow-hidden shadow-lg bg-white">
      ...
      <div className="hidden md:flex flex-col justify-end items-center relative w-[60%] p-0 overflow-hidden border" style={{borderColor: '#e5e7eb', borderWidth: 1, background: '#F3F3FF'}}>
        ...
        <div
          ...
          className="absolute inset-0 w-full h-full z-0 flex items-center justify-center px-8 md:px-8 sm:px-4"
        ...
        >
          <Image
            ...
            alt="Onboarding"
            fill
            style={{objectFit:'contain', zIndex:0, opacity:1}}
          ...
          <div className="flex gap-2 w-full max-w-xs mx-auto mb-2 z-10">
            {slides.map((_, idx) => (
              <div key={idx} className="flex-1 h-1 bg-white/30 rounded overflow-hidden">
                <div
                  className={`h-full transition-all duration-400 ${idx === slide ? 'bg-[#232946]' : 'bg-white/50'}`}
                  style={{width: idx === slide ? `${progress}%` : idx < slide ? '100%' : '0%'}}
                />
              </div>
            ))}
          </div>
      ...
      <div className="flex flex-col justify-center items-center w-full md:w-1/2 p-8 bg-white">
        <div className="w-full max-w-[350px]">
          <div className="flex flex-col items-center mb-6">
            <Image src="/placeholder-logo.svg" alt="Astro Finance Logo" width={120} height={120} className="mb-2" />
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Button type="submit" className="w-full bg-[#232946] hover:bg-[#1a1a2e] text-white" disabled={isLoading}>
